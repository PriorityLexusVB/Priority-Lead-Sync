// Gen-1 (Spark-safe) Functions
import * as functions from "firebase-functions"; // v1 API => Gen-1
...
/**
 * receiveEmailLead
 *  - Auth via x-webhook-secret
 *  - Accepts JSON or ADF/XML
 *  - Writes to Firestore (collection: leads_v2) in named DB 'leads'
 */
  ...
  .https.onRequest(async (req, res) => {
    try {
      ...
      if (ct.includes("application/json")) {
        ...
        // ADF/XML path
        const xml = Buffer.isBuffer(req.rawBody)
      ...
      return res
        .status(400)
        .json({ ok: false, error: "Bad request: " + String(err?.message || err) });
    }
  });

/**
 * listLeads – HTTPS GET, CORS enabled, read-only for Electron polling
 * Query params:
...
export const listLeads = functions
  ...
  .https.onRequest(async (req, res) => {
    try {
      // CORS (Electron file:// → origin 'null')
      ...
      const limitParam = Math.max(
        1,
        Math.min(100, parseInt(String(req.query.limit || "50"), 10))
      );
...
export const generateAIReply = functions
  ...
  .https.onRequest(async (_req, res) => {
    if (!ENV.OPENAI_API_KEY) {
      return res
        .status(200)
        .json({ ok: true, stub: true, note: "No OPENAI_API_KEY set; Spark-safe stub.", gen: "v1" });
    }
    return res.json({
      ok: true,
      note: "OPENAI_API_KEY present. Add outbound call if on Blaze.",
      gen: "v1",
    });
  });
